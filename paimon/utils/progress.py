...
async def progress(
    current: int,
    total: int,
    message: "paimon.Message",
    ud_type: str,
    file_name: str = "",
    c_q: CallbackQuery = None,
    delay: int = paimon.Config.EDIT_SLEEP_TIMEOUT,
) -> None:
    """progress function"""
    ...
    if current == total:
        ...
        try:
            if c_q:
                await c_q.edit_message_text("`finalization process...`")
            else:
                await message.try_to_edit("`finalization process...`")
    ...
    if (now - last) >= delay:
        ...
        progress_str = progress_str.format(
            ...
            "".join(
                (
                    paimon.Config.FINISHED_PROGRESS_STR
                    for i in range(floor(percentage / 5))
                )
            ),
            "".join(
                (
                    paimon.Config.UNFINISHED_PROGRESS_STR
