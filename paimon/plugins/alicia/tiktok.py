...
from paimon import Message, paimon, Config
...
async def ttdown_(message: Message):
    ...
    if not response["status"] == 200:
        return await message.err("There was an error querying the video data, check if you entered the link correctly.", del_in=10)
