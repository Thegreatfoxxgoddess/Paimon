...
from ..helpers.stop import forbidden_sudo
...
async def add_post(message: Message):
    ...
    try:
        chat_ = await paimon.get_chat(chat_)
...
async def del_post(message: Message):
    ...
    if "-all" in message.flags:
        ...
        try:
            async with paimon.conversation(message.chat.id) as conv:
...
async def post_(message: Message):
    ...
    if ("-all" or "-grp" or "-pvt") not in flags:
        ...
        if target.isdigit():
            target = int(target)
        try:
            chat_ = await paimon.get_chat(target)
        except BaseException:
            await message.edit(
            ...
            return
        if BROAD_TAGGING:
            broad = await paimon.send_message(chat_.id, "#BROADCAST")
    ...
    try:
        async for chats_ in POST_LIST.find():
            if "-all" in flags:
                if BROAD_TAGGING:
                    broad = await paimon.send_message(chats_["chat_id"], "#BROADCAST")
                    broad_id = broad.message_id
                else:
                    broad_id = None
                await paimon.copy_message(
                ...
                if chats_["chat_type"] in ["group", "supergroup"]:
                    if BROAD_TAGGING:
                        broad = await paimon.send_message(
