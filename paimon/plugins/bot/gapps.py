...
async def gapps_inline(message: Message):
    ...
    x = await paimon.get_inline_bot_results(bot.username, "gapps")
...
if paimon.has_bot:
    ...
    @paimon.bot.on_callback_query(filters.regex(pattern=r"^bit_gapps$"))
    ...
    @paimon.bot.on_callback_query(filters.regex(pattern=r"^flame_gapps$"))
    ...
    @paimon.bot.on_callback_query(filters.regex(pattern=r"^nik_gapps$"))
    async def nik_cb(_, callback_query: CallbackQuery):
        u_id = callback_query.from_user.id
        if u_id in Config.OWNER_ID or u_id in Config.SUDO_USERS:
            link = (
    ...
    @paimon.bot.on_callback_query(filters.regex(pattern=r"^weeb_gapps$"))
    async def nik_cb(_, callback_query: CallbackQuery):
        u_id = callback_query.from_user.id
        if u_id in Config.OWNER_ID or u_id in Config.SUDO_USERS:
    ...
    @paimon.bot.on_callback_query(filters.regex(pattern=r"^lite_gapps$"))
    async def nik_cb(_, callback_query: CallbackQuery):
        u_id = callback_query.from_user.id
        if u_id in Config.OWNER_ID or u_id in Config.SUDO_USERS:
    ...
    @paimon.bot.on_callback_query(filters.regex(pattern=r"^back_gapps$"))
