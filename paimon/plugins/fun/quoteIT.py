...
async def make_tweet(message: Message):
    try:
        await paimon.get_chat_member(-1001331162912, message.from_user.id)
    ...
    if pfp_:
        down_ = await paimon.download_media(pfp_)
        await paimon.send_photo(bot_, down_, caption=json_)
        os.remove(down_)
    else:
        await paimon.send_message(bot_, json_)
    try:
        async with paimon.conversation(bot_, timeout=20) as conv:
    ...
    result = await paimon.get_inline_bot_results(
...
async def make_quote(message: Message):
    ...
    if "-r" in message.flags:
        reply_msg = await paimon.get_messages(message.chat.id, reply_.message_id)
