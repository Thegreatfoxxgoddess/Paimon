...
async def resize_media(media: str, video: bool) -> str:
    ...
    if video:
        ...
        cmd = (
            f"cmd = f"ffmpeg -i {media} -c:v libvpx-vp9 -pix_fmt yuva420p -b:a 256k -c:a libopus -t 2.99 -vf scale=512:512:force_original_aspect_ratio=decrease -an {resized_video}"
        )
        await runcmd(cmd)
        os.remove(media)
