...
async def ytDown(message: Message):
    ...
    if bool(message.flags):
        ...
        if "m" in message.flags:
            retcode = await _mp3Dl([message.filtered_input_or_reply_str], __progress, startTime)
            ...
            retcode = await _tubeDl(
                [message.filtered_input_or_reply_str], __progress, startTime, desiredFormat
            )
            ...
            retcode = await _tubeDl([message.filtered_input_or_reply_str], __progress, startTime)
    else:
        retcode = await _tubeDl([message.filtered_input_or_reply_str], __progress, startTime)
    if retcode == 0:
        _fpath = ""
