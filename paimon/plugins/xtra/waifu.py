...
async def anime_sticker(message: Message):
    ...
    if "-g" in message.flags:
        try:
            stickers = await paimon.get_inline_bot_results(
    ...
    try:
        ...
        saved = await paimon.send_inline_bot_result(
            chat_id="me",
            query_id=stickers.query_id,
            result_id=stickers.results[0].id,
            hide_via=True,
        )
        saved = await paimon.get_messages("me", int(saved.updates[1].message.id))
