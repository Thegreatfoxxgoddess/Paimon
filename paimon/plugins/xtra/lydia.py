...
from paimon import Message, filters, get_collection, paimon, pool
from paimon.utils import get_file_id

LOGGER = paimon.getCLogger(__name__)
...
async def _init():
    ...
    if CUSTOM_REPLY_CHANNEL:
        async for message in paimon.iter_history(
    ...
    # Group Features Won't be displayed in Help Info For Now 😉
        ...
        for user_id in ACTIVE_CHATS:
            if str(user_id).startswith("-100"):
                continue
            try:
                u_info = await paimon.get_user_dict(user_id)
...
async def lydia_ai_chat(message: Message):
    ...
    if data:
        ...
        try:
            out = ""
            await paimon.send_read_acknowledge(
...
# Idea arised from here (https://t.me/paimonot/157629) thnx 👍
