...
from paimon.utils import terminate
...
async def restart_(message: Message):
    """restart paimon"""
    await message.edit("`Reiniciando serviços do paimon`", log=__name__)
    LOG.info("Serviços paimon - Reinício iniciado")
...
@paimon.on_cmd(
    "shutdown", about={"header": "Desligar o paimon :)"}, allow_channels=False
)
...
async def die_(message: Message) -> None:
    """set offline timeout to die paimon"""
    global MAX_IDLE_TIME  # pylint: disable=global-statement
...
@paimon.on_cmd(
    "sleep (\\d+)",
    about={"header": "sleep paimon :P", "usage": "{tr}sleep [timeout in seconds]"},
    allow_channels=False,
)
